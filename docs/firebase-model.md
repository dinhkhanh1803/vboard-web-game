...
| leaderboardEntries/{entryId}               | Public Elo and stats row by game for leaderboard reads.                 |
| matchHistoryEntries/{entryId}              | Per-player completed match history entries for profile timelines.       |
...
Phase 12 pins the ranking and match-history data language in `contracts/progression.ts`. These contracts are TypeScript-only for now; no live Firestore reads or writes exist yet.

### `leaderboardEntries/{entryId}`

Public leaderboard row generated by server-owned progression workflows.

| Field            | Type                    | Notes                                    |
| ---------------- | ----------------------- | ---------------------------------------- |
| `uid`            | `string`                | Player UID.                              |
| `displayName`    | `string`                | Public profile display name.             |
| `avatarUrl`      | `string \| null`        | Public avatar URL when available.        |
| `gameId`         | `"connect-4" \| "caro"` | Game this row ranks.                     |
| `rank`           | `number`                | Display rank in the current preview set. |
| `elo`            | `number`                | Current rating for the game.             |
| `wins`           | `number`                | Win count for the game.                  |
| `losses`         | `number`                | Loss count for the game.                 |
| `draws`          | `number`                | Draw count for the game.                 |
| `gamesPlayed`    | `number`                | Completed rated matches for the game.    |
| `winRatePercent` | `number`                | Rounded public win rate.                 |
| `updatedAtMs`    | `number`                | Unix epoch milliseconds.                 |

### `matchHistoryEntries/{entryId}`

Per-player match history entry generated from completed matches.

| Field                 | Type                        | Notes                                            |
| --------------------- | --------------------------- | ------------------------------------------------ |
| `id`                  | `string`                    | `${matchId}_${playerUid}` for deterministic IDs. |
| `matchId`             | `string`                    | Completed match ID.                              |
| `gameId`              | `"connect-4" \| "caro"`     | Game identifier.                                 |
| `playerUid`           | `string`                    | Profile owner UID.                               |
| `playerDisplayName`   | `string`                    | Profile owner display name at completion.        |
| `opponentUid`         | `string`                    | Opponent UID.                                    |
| `opponentDisplayName` | `string`                    | Opponent display name at completion.             |
| `result`              | `"win" \| "loss" \| "draw"` | Result from this player's perspective.           |
| `reason`              | `MatchResultReason`         | Official match completion reason.                |
| `eloBefore`           | `number`                    | Rating before this match.                        |
| `eloAfter`            | `number`                    | Rating after this match.                         |
| `eloDelta`            | `number`                    | Rating delta for display and audit.              |
